...
5. **会话ID**: 为了保持上下文的连续性，会在输出内容中出现`<CONVERSATION_ID>...</CONVERSATION_ID>`的前后缀内容，处在中间的文本即为会话ID。当用户下一个提问时，将该会话ID以`-c`的参数形式传入给python脚本。
6. **前置消息ID**: 为了保持上下文的连续性，会在输出内容中出现`<MESSAGE_ID>...</MESSAGE_ID>`的前后缀内容，处在中间的文本即为前置消息ID。当用户下一个提问时，将该ID以`-p`的参数形式传入给python脚本。

### 通用参数

| 参数   | 说明                                          | 示例                                         |
|------|---------------------------------------------|--------------------------------------------|
| `-q` | 必填。问题内容（用户输入的文本），**完整传入用户输入的内容，不做任何的修改和添加** | `-q '@心动小镇 2025-12-18日的收入是多少？' `           |
| `-c` | 可选。会话ID                                     | `-c 8427f4f5-97be-44ce-9b80-744715399367 ` |
| `-p` | 可选。前置消息ID                                   | `-p f1df654d-5f13-4ca0-9864-46b60e011bf5 ` |
